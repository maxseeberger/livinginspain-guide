---
title: "Modelo 210: Spain's Annual Non-Resident Property Tax"
description: "Complete guide to filing Modelo 210 — the annual tax every non-resident property owner in Spain must pay."
category: "Taxes & Finance"
slug: "modelo-210"
lastUpdated: "2026-05-15"
readingTime: 12
simplifySimple: |
  Every non-resident who owns property in Spain must pay this tax every year — even if you never rent the place out. Spain assumes you earn a small "phantom rent" just by owning it, and taxes that notional income. For a German owner, the rate is 19% of 1.1% of your property's cadastral value (a value set by the council, usually well below what you paid). On a typical Mallorca holiday home, this works out to a few hundred euros a year. Most owners hand it to a gestoría who files it for them before the 31 December deadline.
simplifyBullet: |
  • **Deadline:** File and pay by 31 December each year for the previous tax year.
  • **Cadastral value:** Tax is based on 1.1% of your cadastral value — not what you paid for the property.
  • **Tax rate:** 19% if you are an EU/EEA resident; 24% if you are from outside the EU (e.g. post-Brexit UK).
  • **No rental needed:** You must file even if the property sits empty all year — imputed income is always taxable.
  • **Gestoría recommended:** Most non-residents use a gestoría to file; fees are typically €100–€200 per property.
simplifyExample: |
  German owner, Mallorca holiday home, purchase price €400,000.
  Cadastral value (valor catastral): €160,000.
  Step 1 — Calculate imputed income: €160,000 × 1.1% = €1,760.
  Step 2 — Apply the tax rate: €1,760 × 19% = €334.40.
  Annual Modelo 210 tax bill: €334.40. Due by 31 December.
---

<SummaryBox>
Every non-resident who owns property in Spain must file Modelo 210 once a year, even if you never rent the property out. For 2025, the deadline is **31 December 2026**. The tax is calculated on your property's cadastral value and is typically a few hundred euros per year. Most owners use a gestoría to file on their behalf.
</SummaryBox>

## What is Modelo 210?

Modelo 210 is Spain's annual income tax declaration for non-residents (IRNR — Impuesto sobre la Renta de No Residentes). If you own property in Spain but do not live there as a tax resident, you owe this tax every year — full stop.

The logic behind it: Spanish tax law assumes that owning a property generates a deemed "imputed rental income" even if you never rent it out. You are taxed on this notional income.

<Callout variant="warning" title="You must file even if you don't rent">
Owning a Spanish property and not filing Modelo 210 leaves you exposed to penalties and interest. Hacienda (Spain's tax authority) has access to land registry data and actively cross-references it with tax returns.
</Callout>

## Who must file?

You must file Modelo 210 if you are **all** of the following:
- A non-resident of Spain (you spend fewer than 183 days per year in Spain)
- An owner of Spanish property (as registered on the escritura)
- Not already paying Spanish income tax as a resident (Modelo 100)

If you jointly own property with a spouse or partner, **each owner must file separately** — even if the tax is tiny.

## How is the tax calculated?

For a property you use personally (not rented), the calculation has two steps:

**Step 1 — Calculate imputed rental income**

The imputed rental income is a percentage of the cadastral value (valor catastral):
- **1.1%** of the cadastral value if the value was revised after 1 January 1994
- **2%** of the cadastral value if the value has never been revised (older cadastral values)

Most properties in Mallorca use the 1.1% rate.

**Step 2 — Apply the tax rate**

<DataTable
  headers={["Your tax residence", "Tax rate on imputed income"]}
  rows={[
    ["EU / EEA residents (Germany, Netherlands, etc.)", "19%"],
    ["UK residents (post-Brexit)", "24%"],
    ["US, Canada, Switzerland, and others", "24%"],
  ]}
  caption="Modelo 210 tax rates by residency — 2026"
/>

**Example: German owner, €400,000 property, cadastral value €180,000**

1. Imputed income: €180,000 × 1.1% = **€1,980**
2. Tax: €1,980 × 19% = **€376.20 per year**

<Callout variant="tip" title="Cadastral value is not market value">
The cadastral value (valor catastral) is set by the local municipality and is always substantially lower than the market value of your property. You can find your cadastral value on your IBI receipt, or via the Catastro website using your property reference number (referencia catastral).
</Callout>

## If you rent your property out

If you rent the property (even occasionally via Airbnb), you must declare **actual rental income** rather than imputed income on Modelo 210. EU/EEA residents can deduct eligible expenses (mortgage interest, management fees, repairs). Non-EU residents (UK, US) cannot deduct expenses — they pay 24% on gross rental income.

You must file a **separate Modelo 210 for each quarter** in which you received rental income.

## Key deadlines

<DataTable
  headers={["What", "Deadline"]}
  rows={[
    ["Modelo 210 for imputed income (full year 2025)", "31 December 2026"],
    ["Modelo 210 for rental income (Q1 2026)", "20 April 2026"],
    ["Modelo 210 for rental income (Q2 2026)", "20 July 2026"],
    ["Modelo 210 for rental income (Q3 2026)", "20 October 2026"],
    ["Modelo 210 for rental income (Q4 2026)", "20 January 2027"],
  ]}
/>

## How to file

There are three ways to file Modelo 210:

**1. Use a gestoría (recommended)**
A gestoría is a Spanish administrative agent who files on your behalf. Costs typically €80–€200 per year. This is by far the easiest option and eliminates the risk of errors.

**2. File online yourself**
You can file through the Agencia Tributaria website if you have an [FNMT digital certificate](/taxes/digital-certificate) or Cl@ve PIN. The process requires:
- Your NIE number (see [Modelo 030](/taxes/modelo-030) if you haven't activated yours)
- Your property's referencia catastral
- Your cadastral value
- Your bank account for direct debit (or you can print the form and pay at a bank)

**3. File by paper**
Paper filing is still accepted at Spanish bank branches. Practically speaking, the online route is easier if you already have a digital certificate.

<Callout variant="info" title="Joint owners file separately">
If you own 50/50 with your partner, each of you files their own Modelo 210 for their 50% share of the imputed income. The total tax is the same — it's just split across two declarations.
</Callout>

## What about years I haven't filed?

If you've owned property in Spain but haven't been filing Modelo 210, you are in arrears. The statute of limitations is 4 years, so Hacienda can theoretically go back 4 years. The penalty for late filing ranges from 5% to 20% of the tax owed, plus interest of around 3.75% per year.

Many owners in this situation file the outstanding returns voluntarily (called a regularización voluntaria) — this reduces penalties significantly compared to being caught in a Hacienda audit.

A gestoría can help you catch up on unfiled years efficiently.

<CTABox />

<OfficialSources
  sources={[
    { label: "Agencia Tributaria — Modelo 210 filing portal", href: "https://sede.agenciatributaria.gob.es/Sede/procedimientoini/GI30.shtml" },
    { label: "Catastro — find your referencia catastral and cadastral value", href: "https://www.sedecatastro.gob.es/" },
    { label: "Ley del IRNR (Non-Resident Income Tax Law)", href: "https://www.boe.es/buscar/act.php?id=BOE-A-2004-21427" },
  ]}
/>

<RelatedGuides
  guides={[
    { title: "Modelo 030: Activating Your NIE as a NIF", href: "/taxes/modelo-030" },
    { title: "FNMT Digital Certificate: File Online", href: "/taxes/digital-certificate" },
    { title: "IBI: Spain's Local Property Tax", href: "/taxes/ibi" },
    { title: "Germany–Spain Double Taxation Agreement", href: "/taxes/germany-spain-dta" },
    { title: "UK–Spain Double Taxation Treaty", href: "/taxes/uk-spain-dta" },
    { title: "Global Consulting Mallorca (professional help)", href: "/professionals/global-consulting-mallorca" },
  ]}
/>
