...
readingTime: 7
simplifySimple: |
  The TIE is the modern biometric residence card that replaces the old paper NIE certificate for people who actually live in Spain. If you're a non-resident who just owns property, you don't need one. You only get a TIE when you register as a resident — either as an EU citizen or through a visa.
simplifyBullet: |
  • **Biometric plastic card for residents only:** credit-card sized with photo and fingerprints
  • **Replaces old paper NIE certificate:** but the underlying NIE number stays the same
  • **EU citizens get EU-citizen TIE:** through the EU citizen registration process
  • **Non-EU citizens get non-EU TIE:** after receiving a visa (NLV, Digital Nomad, etc.)
  • **Non-residents keep paper NIE:** no TIE needed if you just own property and don't live in Spain
simplifyExample: |
  German owner retires to Mallorca full-time. She registers as an EU citizen resident at the foreigner's office (oficina de extranjeros) with her passport and empadronamiento certificate. She gets a TIE card with her photo and fingerprints. Her old paper NIE number stays the same — just on a card now.
---
