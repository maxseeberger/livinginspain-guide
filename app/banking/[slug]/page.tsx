...
export async function generateStaticParams() {
  return getAllArticles('banking').map((a) => ({ slug: a.slug }));
}
...
export default async function BankingArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  let article;
  try { article = getArticle('banking', slug); } catch { notFound(); }
