...
export async function generateStaticParams() {
  return getAllArticles('banking', 'de').map((a) => ({ slug: a.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  try {
    const { slug } = await params;
    const article = getArticle('banking', slug, 'de');
    return { title: article.title, description: article.description };
  } catch { return {}; }
}

export default async function DeBankingArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  let article;
  try { article = getArticle('banking', slug, 'de'); } catch { notFound(); }
  ...
  return (
    <ArticleLayout title={article.title} description={article.description} category={article.category}
      ...
      breadcrumb={[{ label: 'Banking & Finanzen', href: '/de/banking' }]}>
