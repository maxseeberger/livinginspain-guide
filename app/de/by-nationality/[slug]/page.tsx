...
export async function generateStaticParams() {
  return getAllArticles('by-nationality', 'de').map((a) => ({ slug: a.slug }));
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  try {
    const { slug } = await params;
    const article = getArticle('by-nationality', slug, 'de');
    return { title: article.title, description: article.description };
  } catch { return {}; }
}

export default async function DeByNationalityArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  let article;
  try { article = getArticle('by-nationality', slug, 'de'); } catch { notFound(); }
  ...
  return (
    <ArticleLayout title={article.title} description={article.description} category={article.category}
      ...
      breadcrumb={[{ label: 'Nach Nationalität', href: '/de/by-nationality' }]}>
